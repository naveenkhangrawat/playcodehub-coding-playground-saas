...
function ThemeSelector() {
    ...
    const themeInfo = themes.find((thm) => thm.id === theme);
    ...
    return (
        <div className='relative' ref={dropDownRef}>
            ...
            >
                ...
                <Palette className="w-5 h-5 text-gray-400 group-hover:text-gray-300 transition-colors"/>
                    ...
                    >
                        ...
                        {themes?.map((thm, index) => (
                            <motion.button
                                ...
                                className={`relative group w-full flex items-center gap-3 px-3 py-2.5   hover:bg-[#262637] transition-all duration-200 ${theme === thm.id ? "bg-blue-500/10 text-blue-400" : "text-gray-300"}`}
                                onClick={() => {
                                    localStorage.setItem("editor-theme", thm.id);
                                    dispatch(setTheme(thm.id));
                                }}
                            >
                                {/* bg gradient */}
                                ...
                                {/* icon */}
                                <div className={`flex items-center justify-center size-8 rounded-lg         ${theme === thm.id ? "bg-blue-500/10 text-blue-400" : "bg-gray-800/50 text-gray-400"}group-hover:scale-110 transition-all duration-200`}>
                                    {THEME_ICONS[thm.id] || <CircleOff className="w-4 h-4" />}
                                </div>
