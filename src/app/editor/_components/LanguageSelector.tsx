...
import { setError, setExecutionResult, setLanguage, setOutput } from '@/reduxTk/editorSlice';

interface LanguageSelectorProps {
    isProMember: boolean
}

function LanguageSelector({isProMember}: LanguageSelectorProps) {
    ...
    const languageSelectHandler = (langId: string) => {
        if(!isProMember && langId !== "javascript") return;

        setIsOpen(false);
        localStorage.setItem("editor-language", langId);
        dispatch(setLanguage(langId));
        dispatch(setError(null));
        dispatch(setOutput(""));
        dispatch(setExecutionResult(null));
    }
    ...
    useEffect(() => {
        if(!isProMember && language !== "javascript"){
            localStorage.setItem("editor-language", "javascript");
    ...
    return (
        <div className='relative' ref={dropDownRef}>
            <motion.button
                ...
                rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700 ${!isProMember && language !== "javascript" ? "opacity-50 cursor-not-allowed" : ""}`}
            >
                {/* decoration */}
                <div 
                    ...
                    >
                        ...
                        <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
                            {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                                const isLocked = !isProMember && lang.id !== "javascript";
                                return (
                                    <motion.div
                                        key={lang.id}
                                        initial={{ opacity: 0 }}
                                    ...
                                    >
                                        <button
                                            ...
                                            onClick={() => {languageSelectHandler(lang.id)}}
                                            disabled={isLocked}
                                        >
                                            {/* decorator */}
