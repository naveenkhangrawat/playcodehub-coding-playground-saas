...
function OutputPanel() {
    ...
    const error = useAppSelector((state) => state.codeEditor.error);
    const output = useAppSelector((state) => state.codeEditor.output);
    const isRunning = useAppSelector((state) => state.codeEditor.isRunning);

    async function handleCopy(){
        if(!(error || output)) return 
        await window.navigator.clipboard.writeText(error || output);
        setIsCopied(true);
    ...
    return (
        <div className="relative bg-[#181825] rounded-xl p-4 ring-1 ring-gray-800/50">
            ...
            <div className="relative">
                ...
                rounded-xl p-4 h-[633px] overflow-auto font-mono text-sm">
                    {isRunning ? (
                        <RunningCodeSkeleton />
                    ) : error ? (
